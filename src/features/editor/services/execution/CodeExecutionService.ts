...
import { ExecutionStrategy } from './interfaces';
...
export class CodeExecutionService {
  ...
  private initializeStrategies(user: User | null, judge0Url: string): void {
    ...
    this.register('javascript', [
      ...
      new Judge0Backend('javascript', judge0Url, JUDGE0_LANGUAGE_IDS.javascript, user)
    ]);
   ...
   */
  private register(language: string, backends: any[]): void {
   ...
   */
  getLanguageStatus(): Record<string, any> {
  ...
  private formatOutput(result: CodeExecutionResult, mode: ExecutionMode): string {
    ...
    const resultSummary = mode.type === 'RUN' 
