...
describe('WasmManager', () => {
  ...
  describe('isLoaded', () => {
    it('should return false for unloaded runtime', () => {
      const manager = new WasmManager([pythonRuntime]);
      
      expect(manager.isLoaded('python')).toBe(false);
    });
    ...
    it('should return false for unsupported language', () => {
      ...
      expect(manager.isLoaded('unsupported')).toBe(false);
    });
  });
  ...
  describe('load', () => {
    ...
    it('should throw error for unsupported language', async () => {
      ...
      await expect(manager.load('unsupported'))
    ...
    it('should handle load failures', async () => {
      ...
      await expect(manager.load('failing'))
  ...
  describe('runCode', () => {
    ...
    it('should execute code successfully', async () => {
      ...
      const result = await manager.runCode('python', 'print("test")', testCases);
    ...
    it('should throw error for unsupported language', async () => {
      ...
      await expect(manager.runCode('unsupported', 'code', testCases))
        .rejects.toThrow('Unsupported language: unsupported');
    });

    it('should throw error for unloaded runtime', async () => {
      const manager = new WasmManager([pythonRuntime]);
      
      await expect(manager.runCode('python', 'code', testCases))
        .rejects.toThrow('Runtime for python is not loaded yet');
    });
  });
  ...
  describe('getRuntimeStatus', () => {
    it('should return status of all runtimes', async () => {
      ...
      expect(status.javascript).toEqual({ loaded: false, language: 'javascript' });
    });
  });
  ...
  describe('loadAll', () => {
    ...
    it('should handle individual runtime failures gracefully', async () => {
      ...
      expect(manager.isLoaded('python')).toBe(true);
      expect(manager.isLoaded('failing')).toBe(false);
    });
  });
